...
function Server(app){
    ...
    this.browsers = new Backbone.Collection
    this.browsers.on('remove', function(browser){
        browser.unbind()
    })
    ...
    , initServer: function(){
        ...
        exp.get('/', function(req, res){
            var framework = config.get('framework') || 'jasmine'
              ...
              , browserify = config.get('browserify')
        ...
        exp.get('/testem.js', function(req, res){
            var isBrowserify = config.get('browserify')
                , src_files = config.get('src_files')
            ...
            async.forEachSeries(files, function(file, done){
                ...
                if (isBrowserify) {
                    browserifyResponse(src_files, res)
                } else {
                    res.end()
                }
            })
...
function browserifyResponse(files, res) {
    var browserifyPath = path.join(__dirname, "..", "node_modules",
        ".bin", "browserify")

    expandGlobs(files, function (err, files) {
        if (err) {
            throw err
        }
        ...
        var child = spawn(browserifyPath, files, {
            cwd: process.cwd(),
            env: process.env
        })
...
function expandGlobs(files, callback)  {
    async.reduce(files, [], function(curr, pattern, next){
