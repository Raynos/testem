...
var log = require('winston')
  ...
  , isOkAndNumber = /^ok\s\d/
  , isTapVersion = /TAP version \d/
  , isTapNumberRange = /\d\.\.\d/
...
var ProcessRunner = Backbone.Model.extend({
  ...
  , registerProcess: function(process){
    ...
    var outLines = stdout.pipe(split())

    if (!settings.hide_stdout){
      outLines.on('data', function(data){
        if (self.isTap() && self.hideIfTap(data)) {
          return
        }
  ...
  , hideIfTap: function (chunk) {
    var str = String(chunk)
    if (str[0] === "#") {
      return true
    } else if (isOkAndNumber.test(str)) {
      return true
    } else if (isTapVersion.test(str)) {
      return true
    } else if (isTapNumberRange.test(str)) {
      return true
    }
  }
